...
if ! declare -f _fzf_compgen_path > /dev/null; then
  _fzf_compgen_path() {
    echo "$1"
    command find -L "$1" \
      -name .git -prune -o -name .svn  -prune -o -name .npm -prune -o -name \
      node_modules -prune -o \( -type d -o -type f -o -type l \) \
...
_fzf_complete_kill() {
  local selected_pid
  selected_pid=$(ps -ef | sed 1d | awk '{print $2, $8}' | fzf --preview="" --height=50% --reverse --select-1 --header='[Process to kill]') && kill -${1:-TERM} $selected_pid
  LBUFFER="kill -kill $selected_pid"   # Append result to command line buffer
  zle reset-prompt                   # Reset prompt
}
